"""Train a plain DGCNN model from scratch on your own quick20-recorded,
self-labeled data -- same feature pipeline, split methodology, and label
collapse as train_dagcn_own_data.py, just swapping the classifier so results
are directly comparable.

Usage:
  "C:/Users/USER/miniconda3/envs/EEG/python.exe" train_dgcnn_own_data.py --split-by session
"""
...
def main():
    ...
    out_dir = Path(args.out_dir) if args.out_dir else HERE / f"result_dgcnn_own_data_split_{args.split_by}"
    ...
    # layers explicit (not via yaml relative-path lookup, which fails from this cwd) --
    # 19 electrodes hits neither the num_electrodes==62/32 special case, so pass it directly.
    ...
    print("\n=== Training DGCNN from scratch on your own labeled quick20 data ===")
    metric = libeer_train(
