...
Usage:
  "C:/Dev/BCI/LibEER/.venv39/Scripts/python.exe" seed_montage_classify_persubj_trunc.py
  "C:/Dev/BCI/LibEER/.venv39/Scripts/python.exe" seed_montage_classify_persubj_trunc.py --feat de
"""
...
def extract_subject(f, feat="lds"):
    """DE (or DE_LDS) per trial, all 62 channels, for one subject's one session."""
    m = sio.loadmat(f)
...
def main():
    ...
    for s_i, (sub, f) in enumerate(seed_one_session_per_subject()):
        X, y, tri = extract_subject(f, a.feat)
        for name, chs in MONTAGES.items():
    ...
    out_name = f"seed_montage_cls_persubj_trunc{suffix}.json"
