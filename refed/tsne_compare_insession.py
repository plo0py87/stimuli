"""Same t-SNE separability check, but restricted to a SINGLE session on each
side -- no pooling across SEED's 9 subject-session files or REFED's 32
subjects. Pooling mixes in subject/session-level baseline differences that
can dominate the embedding and have nothing to do with class separability;
this project has already established (in the DGCNN normalization discussion)
that in-session is the right level to look at, not pooled-across-sessions.

SEED: exactly ONE subject-session file (15 trials, ~3000-3500 one-second
samples). REFED has only one session per subject to begin with, so "in
session" here means ONE subject's own 15 clips (~1200-1600 samples).

Both use DE_LDS (non-causal LDS smoothing), per the last request.

Usage:
  "C:/Dev/BCI/LibEER/.venv39/Scripts/python.exe" tsne_compare_insession.py
"""
...
def main():
    print("Loading SEED (1 session, DE_LDS)...")
    Xs, ys = load_seed_one_session()
    print("\nLoading REFED (1 subject, DE_LDS)...")
    ...
    for ax, emb, y, title, sh, se in (
        (axes[0], emb_s, ys, "SEED -- ONE session (15 trials)", sh_s, se_s),
        (axes[1], emb_r, yr, "REFED -- ONE subject (valence, +/-40/127)", sh_r, se_r),
    ):
        for c in range(3):
            m = y == c
    ...
    fig.suptitle("t-SNE, DE_LDS, single session/subject only -- no cross-subject "
                 "pooling", fontsize=12.5, y=1.02)
    fig.tight_layout()
    out = HERE / "fig_tsne_insession.png"
