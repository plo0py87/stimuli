"""Multi-trial true-vs-predicted timeline for the LibEER+non-causal-LDS
regression predictions -- the pipeline we're standardizing on.

fig_timeline() in refed_montage_figures.py only ever showed one hand-picked
"best case" trial. This picks one trial per emotion target category (MVMA/
LVLA/LVHA/HVHA/HVLA), each at the subject closest to the MEDIAN whole-cap
valence CCC for that clip -- a typical case, not a cherry-picked best one --
so the figure is representative rather than flattering.

Usage:
  "C:/Dev/BCI/LibEER/.venv39/Scripts/python.exe" fig_timeline_lds.py
"""
...
def main():
    pz = np.load(HERE / "libeer_lds_all_pred.npz")
    ...
    for r, (target, sub, vid, c) in enumerate(trials):
        ...
        for c_i, dim in enumerate(DIMS):
            ...
            for name in MONT_ORDER:
                p = pz[f"pred_pooled_{name}"][m][order][:, c_i]
    ...
    fig.suptitle("LibEER + non-causal LDS: predicted vs actual, one trial per "
                 "emotion target (median-performance subject, not cherry-picked)",
                 fontsize=12.5, y=1.0)
    fig.tight_layout()
    out = HERE / "fig_timeline_lds_grid.png"
