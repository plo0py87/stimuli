"""Per-person silhouette (plain DE, no LDS), SEED vs REFED -- was subject 9's
in-session separability typical, or a lucky pick?

SEED: one session per subject (first by date), 15 subjects.
REFED: all 32 subjects, one session each (REFED only has one per subject).
Both: LibEER's exact DE (bandpass + de_extraction), no smoothing, 62ch,
z-scored using that person's OWN data only (in-session, no pooling).

Usage:
  "C:/Dev/BCI/LibEER/.venv39/Scripts/python.exe" silhouette_perperson.py
"""
...
def seed_one_session_per_subject():
    ...
    for sub in sorted(by_subject, key=int):
        yield sub, sorted(by_subject[sub])[0]  # earliest session


def sil_for_seed_file(f):
    m = sio.loadmat(f)
...
def sil_for_refed_subject(z, channels, keep, sub):
    subject = z["subject"]
    m = subject == sub
...
def main():
    print("=== SEED, one session per subject, plain DE ===")
    seed_sils = []
    for sub, f in seed_one_session_per_subject():
        s = sil_for_seed_file(f)
        seed_sils.append(s)
        print(f"  subject {sub:>3s} ({f.name}): silhouette={s:+.3f}")
    seed_sils = np.array(seed_sils)

    print("\n=== REFED, 32 subjects, plain DE ===")
    ...
    for sub in range(1, 33):
        s = sil_for_refed_subject(z, channels, keep, sub)
        if s is None:
            print(f"  subject {sub:2d}: skipped (only one class present)")
            continue
        refed_sils.append(s)
        print(f"  subject {sub:2d}: silhouette={s:+.3f}")
